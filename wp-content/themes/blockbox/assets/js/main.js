...
function mouseenter(elem) {
    elem.addEventListener('mouseover', ()=> {
        elem.firstChild.nextSibling.nextSibling.nextSibling.classList.add('card-up')
    })
}
document.querySelectorAll('.blog-item').forEach(elem=> mouseleave(elem))
function mouseleave(elem) {
    elem.addEventListener('mouseleave', () => {
        elem.firstChild.nextSibling.nextSibling.nextSibling.classList.remove('card-up')
    })
}
...
if(window.innerWidth<992) {
    ...
    document.querySelectorAll('.blog-item').forEach(elem=> mouseenter(elem))
    function mouseenter(elem) {
            elem.firstChild.nextSibling.nextSibling.nextSibling.classList.add('card-up-minwidth')
    }
